...
import type { CollectionReactionResponse, ReactionType } from "@/types";
import { useTranslations } from "next-intl";

interface CollectionReactorsDialogProps {
  visible: boolean;
  onHide: () => void;
  collectionId: string;
}
...
export default function CollectionReactorsDialog({
  visible,
  onHide,
  collectionId,
}: CollectionReactorsDialogProps) {
  ...
  const loadReactors = useCallback(async () => {
    if (!collectionId || hasLoadedRef.current) return;
  ...
  }, [visible, collectionId, loadReactors]);
  ...
  const handleReactorClick = useCallback((reactor: CollectionReactionResponse) => {
    handleHide();
