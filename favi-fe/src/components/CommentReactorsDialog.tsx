...
import type { CommentReactionResponse, ReactionType } from "@/types";
...
export default function CommentReactorsDialog({
  visible,
  onHide,
  commentId,
}: CommentReactorsDialogProps) {
  const t = useTranslations("Reactions");
  const tCommon = useTranslations("Common");
  const router = useRouter();
  const [reactors, setReactors] = useState<CommentReactionResponse[]>([]);
  ...
  const loadReactors = useCallback(async () => {
    if (!commentId || hasLoadedRef.current) return;

    hasLoadedRef.current = true;
    setLoading(true);
    try {
      const data = await commentAPI.getReactors(commentId);
      setReactors(data);
    } catch (error) {
      console.error("Failed to load comment reactors:", error);
      setReactors([]);
    } finally {
      setLoading(false);
    }
  ...
  }, [visible, commentId, loadReactors]);
  ...
  const handleReactorClick = useCallback((reactor: CommentReactionResponse) => {
    handleHide();
